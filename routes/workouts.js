...
//  CREATE WORKOUT 
// FIX: category is now accepted and stored
router.post('/', requireAuth, async (req, res) => {
  const { name, description, significance, steps, video, category } = req.body;
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO workouts (name, description, significance, steps, video, category)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        name,
        description  || null,
...
router.put('/:id', requireAuth, async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      `UPDATE workouts
       SET name = ?, description = ?, significance = ?, steps = ?, video = ?, category = ?
       WHERE id = ?`,
      [
        name,
        description  || null,
...
// CACHE VIDEO URL ON WORKOUT 
// Called automatically after a YouTube search so the same workout is never searched twice
router.patch('/:id/video', requireAuth, async (req, res) => {
  const { id }    = req.params;
